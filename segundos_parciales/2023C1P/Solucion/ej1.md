...
> - 5 tareas nivel 3 (1)
> - 1 tarea nivel 0 (2)
> - (1) escriben en EAX resultados y (2) los puede leer (por dónde le llegan?)
...
_isr80:
    ...
    mov word [selector_tarea_6], ax
    jmp far [offset_tarea_6]
...
```c
while(true){
    uint32_t eax = tss_tasks[current_task].eax; // obtengo eax de la tss de la tarea 6
    procesar(eax); // función que hace cualquier cosa
    
    sched_enable_task(tarea_origen); // despausa la tarea
